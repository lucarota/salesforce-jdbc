...
public class InsertQueryProcessor {
    ...
    public static ResultSet processQuery(ForcePreparedStatement statement, String soqlQuery,
        ...
        try {
            int updateCount = 0;
            ISaveResult[] records = partnerService.createRecords(insertQueryAnalyzer.getFromObjectName(),
                insertQueryAnalyzer.getRecords());
            ...
            for (ISaveResult result : records) {
                if (result.isSuccess()) {
                    ...
                    resultSet.addWarning(insertQueryAnalyzer.getFromObjectName() + " failed to create with error="
